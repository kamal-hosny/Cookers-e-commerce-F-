...
import { FaChevronDown, FaChevronRight, FaFilter, FaTimes } from "react-icons/fa";
...
const categoryFilters: Record<string, Filter[]> = {
  ...
  ovens: [
    {
      title: "color",
      options: ["anthracite", "avena", "mirror", "white","black", "stainless_steel"],
    },
    {
      title: "width",
      options: ["45", "60", "90"],
    ...
    {
      title: "collection",
      options: ["professional", "elio_giant", "country", "elementum", "black", "elio", "alterum"],
    },
    {
      title: "oven_type",
      options: ["gas_with_fan", "gas", "electric", " electric_multifunction", "electric_static", "electric_fan"],
    },
    {
      title: "special_function",
      options: ["rotisserie", "true_fan_cooking", "pizza_300°c", "leavening_40°c", "easy_steam", "idroclean", "booster", "leavening"],
    },
  ],
  hobs: [
    {
    ...
    {
      title: "collection",
      options: ["country", "elementum", "domino", "elio", "alterum"],
    },
    {
      title: "cooking_zone",
      options: [
...
const ProductCategory = () => {
  ...
  const [activeFilters, setActiveFilters] = useState<{filter: string, option: string}[]>([]);
  ...
  const activeBanner = useMemo(() => 
    banners.find((banner) => banner.category === activeCategory) || banners[0],
    [activeCategory]
  );
  ...
  const getMetaValue = useCallback((metaData: MetaDataItem[], key: string): string | null => {
  ...
  const extractRealProduct = useCallback((data: Product): RealProduct => ({
    id: data.id || 0,
    name: data.name,
  ...
  const filteredProducts = useMemo(() => {
    return realProducts.filter((product: RealProduct) => {
      return Object.entries(selectedFilters).every(([filterKey, selectedValue]) => {
        if (!selectedValue) return true;
        
        switch (filterKey) {
          case "color": return product.color === selectedValue;
          case "width": return product.width === selectedValue;
          case "collection": return product.collection === selectedValue;
          case "cooktop_type": return product.cooktop_type === selectedValue;
          case "oven_type": return product.oven_type === selectedValue;
  ...
  const handleFilterChange = useCallback((filterTitle: string, option: string) => {
  ...
  // SEO meta data
  const seoTitle = `Premium ${activeBanner.title.replace("Premium ", "")} | Kitchen Appliances`;
  const seoDescription = `Discover our collection of high-quality ${activeBanner.title.replace("Premium ", "")}. ${activeBanner.description.substring(0, 150)}...`;
  ...
  return (
    <main className="bg-[#eaeaea] min-h-screen">
      <Helmet>
        ...
        <link rel="canonical" href={`https://yourdomain.com/product-category/${activeCategory}`} />
      </Helmet>
          ...
          <div className="mt-4">
            ...
            >
              {currentFilters.map((filter) => (
                <div key={filter.title} className="border-b last:border-b-0 px-4 pb-4 mb-4 last:mb-0 last:pb-0">
                  <button
